...
<?php include("includes/header.php"); ?>
	...
	<?php while($row= $sql->fetch()){ ?>
                 <h3>Meal: <?php echo $cont=$row['title']; ?></h3>
                 <h3>Type:   <?php  if($row['m_type_id']== 1){
					echo "Starter";}
				if($row['m_type_id']== 2){
					echo "Main Course";}
				if($row['m_type_id']== 3){
				...
				if($row['m_type_id']== 5){
                 ...
                 <h3>Category:    <?php if ($row['m_cat_id']== 2){
			echo "Vegetarian";
			}else{
				echo "Non-vegetarian";
                        } ?></h3>
                <h3>Details:</h3> <p class="contents"> <?php echo $row['content']; ?></p>
                <p class="posted"><a href="reviews.php?cont=<?php echo $cont_id=$row['id'];?>">Rate & Review</a> </p>
